...
def load_conllu(file):
    ...
    class UDWord:
        def __init__(self, span, columns, is_multiword):
            ...
            self.columns[DEPREL] = columns[DEPREL].split(":")[0].lower()
        ...
        if not line:
            ...
            def process_word(word):
                if word.parent == "remapping":
                    raise UDError("There is a cycle in a sentence '{}'".format(_encode(word.columns[HEAD])))
            ...
            if len([word for word in ud.words[sentence_start:] if word.parent is None]) != 1:
                raise UDError("There are multiple roots in a sentence '{}'".format(" ".join([_encode(word.columns[FORM]) for word in ud.words[sentence_start:]])))
        ...
        for words in alignment.matched_words:
            if filter_fn is None or filter_fn(words.gold_word):
                if key_fn(words.gold_word, gold_aligned_gold) == key_fn(words.system_word, gold_aligned_system):
                    correct += 1
                # else:
                #   print(" ".join([_encode(i) for i in words.gold_word.columns])
                #         +"\n"
                #         +" ".join([_encode(i) for i in words.system_word.columns]))
...
def my_eval(gold_file, system_file):
    gold_ud = load_conllu_file(gold_file)
    system_ud = load_conllu_file(system_file)
    return evaluate(gold_ud, system_ud)

def print_result(eval):
    print("UPOS F1 Score: {:.2f}".format(100 * eval["UPOS"].f1))
    print("LAS F1 Score: {:.2f}".format(100 * eval["LAS"].f1))
    print("UAS F1 Score: {:.2f}".format(100 * eval["UAS"].f1))

templates = ["1-1", "1-2", "2-1", '2-2', "3-1", "3-2"]

for template in templates:
    path_g = "../golden files/gold_"+template+'.conll'
    path_s = '../system files/stanza/untok_'+template+'.conllu'
    path_u = '../system files/udpipe1/untok_'+template+'.conllu'
    path_u2 = '../system files/udpipe2/untok_'+template+'.conllu'

    print("Result for template "+template+" untokenized: \n")

    print("Stanza")
    print_result(my_eval(path_g, path_s))
    print('\n')

    print("UDPipe1")
    print_result(my_eval(path_g, path_u))
    print('\n')

    print("UDPipe2")
    print_result(my_eval(path_g, path_u2))
    print('\n')

    print("+++++++++++++++++++++++++++++++++++++++++++++")

    path_s = '../system files/stanza/tok_'+template+'.conllu'
    path_u = '../system files/udpipe1/tok_'+template+'.conllu'
    path_u2 = '../system files/udpipe2/tok_'+template+'.conllu'

    print("Result for template "+template+" tokenized: \n")
